...
`sync=True` закреплён явно, хотя совпадает с текущим дефолтом GStreamer: задержка рендеринга определяется PTS и clock pipeline, а не скоростью, с которой upstream доставляет буферы. Поэтому `AppSrc` формирует непрерывные PTS и duration из числа переданных frames. Параметр не гарантирует фиксированную end-to-end задержку: её также определяют PipeWire, согласование устройства и ограниченные очереди pipeline.
